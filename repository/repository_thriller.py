...
from database import new_session, Film_thriller
...
class FilmRepository:
    ...
    async def find_one(cls, film_id: int):
        async with new_session() as session:
            query = select(Film_thriller).where(Film_thriller.id == film_id)
            result = await session.execute(query)
            film = result.scalar()
            return film
    ...
    async def update_one(cls, data: FilmAdd, film_id: int):
        async with new_session() as session:
            film = await session.get(Film_thriller, film_id)
            if film is None:
                return None
            film.name = data.name
